...
if systype == 'Windows':
    ...
    if struct.calcsize("P") * 8 == 64:
        dll_path = os.path.join(os.getcwd(), "RyhandLibx64.dll")
    else:
        dll_path = os.path.join(os.getcwd(), "RyhandLibx86.dll")
    ...
    if 'arm' in cpuarch or 'aarch64' in cpuarch:
        if 'arm' in cpuarch :
            lib = ctypes.CDLL('./libRyhand_arm.so')
        elif 'aarch64' in cpuarch:
            lib = ctypes.CDLL('./libRyhand_arm64.so')
    else:
        # System Type: Linux, CPU Architecture: x86_64
        # 默认使用 x86 或 x86_64 架构
        if struct.calcsize("P") * 8 == 64:
            lib = ctypes.CDLL('./libRyhand64_1.so')
        else:
            lib = ctypes.CDLL('./libRyhand.so')
...
if USE_RS485:
    ...
    if  USE_CANFD:
        bus = can.interface.Bus(channel='can0', interface='socketcan', fd=True, bitrate=1000000, data_bitrate=5000000)
    else:
...
def main_menu():
    while True:
        ...
        try:
            ...
            if choice == '1':
                get_motor_angles(3)#id
            elif choice == '2':
                set_speed_control_mode(3,2000,50)#id, speed, target_angle
            elif choice == '3':
                ...
                set_speed_control_mode(1, 2000, 10)
                set_speed_control_mode(2, 2000, 10)
                set_speed_control_mode(3, 2000, 50)
                set_speed_control_mode(4, 2000, 50)
                set_speed_control_mode(5, 2000, 50)
                set_speed_control_mode(6, 2000, 50)
